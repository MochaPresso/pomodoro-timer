...
const borderBox = document.querySelector(".border-clock");
...
(function minuteCircle() {
  ...
  function getRotation(e) {
    ...
    // console.log(rotateCircle.offsetWidth, rotateCircle.offsetHeight);
    // console.log({ x, y });
  ...
  function mousePos(e, currentElement) {
    ...
    canvasX = e.pageX - totalOffsetX - borderBox.clientLeft;
    canvasY = e.pageY - totalOffsetY - borderBox.clientTop;
...
options.forEach((option) => {
  option.addEventListener("click", () => {
    sizeIndex = parseFloat(option.innerHTML.slice(0, 4));
    handleSelect(option);
    main.style.transform = `scale(${sizeIndex / 100})`;
  });
});
...
function createCircle(element) {
  ...
  let mainOvserber = new MutationObserver(() => {
    console.log(element);
    refreshCanvas(element);
  });
  ...
  mainOvserber.observe(main, option);
